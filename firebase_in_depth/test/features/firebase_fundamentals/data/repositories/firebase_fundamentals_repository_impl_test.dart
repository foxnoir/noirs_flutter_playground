...
void main() {
  ...
  test(
    'fetchDeniedDocument maps PermissionException to PermissionFailure',
    () async {
      const repository = FirebaseFundamentalsRepositoryImpl(
        FakeFirebaseFundamentalsDataSource(
          deniedReadError: PermissionException("False for 'get'"),
        ),
      );

      await expectLater(
        repository.fetchDeniedDocument(),
        throwsA(const PermissionFailure(detail: "False for 'get'")),
      );
    },
  );
}
