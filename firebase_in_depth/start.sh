...
if lsof -nP -iTCP:9099 -sTCP:LISTEN >/dev/null 2>&1; then
  echo "Port 9099 is busy. A leftover Auth emulator is still running."
  echo "Kill it, then start again:"
  echo "  kill \$(lsof -t -iTCP:9099 -sTCP:LISTEN)"
  exit 1
fi
...
args=(emulators:start --only firestore,auth --export-on-exit=./emulator-data)
