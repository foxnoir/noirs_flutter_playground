...
Labs will go into **Firestore** (CRUD, collection group queries, indexes, offline cache, performance guarantees), the **local emulator**, and **Storage** (photo upload). Those labs are not here yet. **Items** is the working sample until they land. Two and Three stay as placeholder routes.
