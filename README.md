...
Every app runs on **iOS** (Simulator: **iPhone 17 Pro**, iOS 26.5). Advanced Concepts, Firebase in Depth, and the Riverpod starter also run on **web**. Firebase in Depth is **web-first** (Chrome DevTools for Firestore traffic). **Firebase Course Lab** (Home with beginner / advanced swap, My Courses create/delete) and **Firebase Fundamentals** (document vs collection, index queries, nested `lessons` vs `collectionGroup('lessons')`, realtime snapshots) are the labs in this playground for now.
...
**Web first** (Chrome). Firestore lab **Firebase Fundamentals**: seeded `courses`, collection vs one document, index queries, nested `lessons` vs `collectionGroup('lessons')`, realtime `snapshots()` + `participants` increment. **My Courses**: tutors create (schema-validated) and delete catalog docs. Last practice app in this playground for now.
